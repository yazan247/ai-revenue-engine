...
export const postgresInvoiceRepository: InvoiceRepository = {
  async list(accountId) {
    const { rows } = await getDb().query<Invoice>(`select id::text, customer_name as "customerName", customer_email as "customerEmail", invoice_number as "invoiceNumber", amount::float8 as amount, currency, due_date::text as "dueDate", status, created_at::text as "createdAt" from invoices where account_id = $1 order by created_at desc`, [accountId]);
    return rows;
  },
  ...
  async updateStatus(accountId, invoiceId, status: InvoiceStatus) {
    const { rows } = await getDb().query<Invoice>(`update invoices set status = $1 where id = $2 and account_id = $3 returning id::text, customer_name as "customerName", customer_email as "customerEmail", invoice_number as "invoiceNumber", amount::float8 as amount, currency, due_date::text as "dueDate", status, created_at::text as "createdAt"`, [status, invoiceId, accountId]);
    return rows[0] ?? null;
  },
